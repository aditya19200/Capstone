...
async def trigger_explanation(
    ...
    if existing is not None:
        ...
        return ExplainTriggerResponse(
            ...
            # Legacy explanations store speaks completed/pending/failed;
            # ExplainTriggerResponse.status is typed against done/pending/
            # processing/failed — same remap as GET /explain/{id} below.
            status="done" if existing["status"] == "completed" else existing["status"],
            message=(
...
async def get_explanation(prediction_id: str) -> ExplainResponse:
    ...
    # The legacy explanations store (mock_db._explanations, kept as-is —
    # see its module docstring) speaks pending/completed/failed.
    # ExplainResponse.status is typed against the newer xai_jobs vocabulary
    # (pending/processing/done/failed) — only "completed" needs remapping.
    response_status = "done" if explanation["status"] == "completed" else explanation["status"]
