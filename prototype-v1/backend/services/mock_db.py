...
def claim_batch_items(n: int) -> List[Dict]:
    """
    Mock equivalent of the claim_batch_items(n) Postgres RPC.

    mock_db is single-threaded (no concurrent workers to race), so this
    just takes the oldest n 'pending' rows across all batches and flips
    them to 'processing' — same observable contract as the real RPC.
    """
    pending = [item for item in _batch_items.values() if item["status"] == "pending"]
    pending.sort(key=lambda item: item["created_at"])
    claimed = pending[:n]
    for item in claimed:
        item["status"] = "processing"
        item["locked_at"] = _now_iso()
    return [deepcopy(item) for item in claimed]


def update_batch_item(item_id: str, **fields) -> Optional[Dict]:
    """Partial update on a batch_items row (predicted_label, status, prediction_id, ...)."""
    item = _batch_items.get(item_id)
    if item is None:
        return None
    item.update(fields)
