...
def list_all() -> List[Dict]:
    ...
    Backs GET /stats/dashboard, which aggregates over the whole set
    (confidence bands, label distribution, recent activity). Fine at
    prototype volume; if the table grows large this should move to
    server-side aggregate queries rather than pulling every row.
    """
    if not is_configured():
        from services import mock_db
        rows = mock_db.list_predictions()
