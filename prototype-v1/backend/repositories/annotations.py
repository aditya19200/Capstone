...
def count_conflicts() -> int:
    """
    Number of annotations flagged has_conflict — i.e. where a human disagreed
    with the model, or two annotators disagreed with each other.

    Used by the dashboard's "Flagged / Conflicts" figure.
    """
    if not is_configured():
        from services import mock_db
        return len(mock_db.list_annotations(has_conflict=True))
