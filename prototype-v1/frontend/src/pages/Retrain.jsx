...
import { getRetrainEligibility, getRetrainStatus, triggerRetrain } from '../api/client.js'
...
function RetrainPage() {
  ...
  const refreshEligibility = async () => {
    ...
    try {
      setEligibility(await getRetrainEligibility())
    } catch (error) {
      console.error('[Retrain] eligibility check failed', error)
    } finally {
  ...
  // A finished run consumes nothing, but reviews submitted in between do move
  // the count — refresh whenever we land back on the idle screen.
  useEffect(() => {
      ...
      {eligibility ? (
        <div className="dashboard-card">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-3xl font-semibold tabular-nums text-slate-900">
                {eligibility.validated_count}
                <span className="ml-2 text-base font-normal text-slate-500">
                  / {eligibility.required} needed
                </span>
              </p>
              <p className="mt-2 text-sm text-slate-600">
                {eligibility.eligible
                  ? 'Threshold met — retraining can run.'
                  : `${eligibility.required - eligibility.validated_count} more human-reviewed annotation(s) needed before retraining is allowed.`}
              </p>
            </div>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              >
                {eligibility.eligible ? 'Eligible' : 'Below threshold'}
              </span>
              <button
                type="button"
              ...
              >
                {isRefreshingCount ? 'Refreshing...' : 'Refresh'}
              </button>
            </div>
          </div>
          ...
          <div className="mt-4 h-2 w-full overflow-hidden rounded-full bg-slate-200">
            <div
              className={eligibility.eligible ? 'h-full bg-green-500' : 'h-full bg-amber-500'}
              style={{
                width: `${Math.min(100, (eligibility.validated_count / eligibility.required) * 100)}%`,
              }}
            />
          </div>
        </div>
